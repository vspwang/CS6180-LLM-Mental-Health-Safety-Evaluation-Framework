...
Usage:
    ...
    python eval/run_eval.py --judge-model qwen/qwen3-5-flash-02-23
"""
...
def _compute_summary(eval_turns: list, scoring_schema: dict) -> dict:
    ...
    # Group field names by range and description — drives what aggregation to apply
    harm_fields     = [f["name"] for f in fields if f.get("range") == "0-2"]
    help_fields     = [f["name"] for f in fields if f.get("range") == "1-5" and "Help" in f.get("description", "")]
    ai_fields       = [f["name"] for f in fields if f.get("range") == "1-5" and "AI Quality" in f.get("description", "")]
    referral_fields = [f["name"] for f in fields if "or NA" in f.get("range", "")]
