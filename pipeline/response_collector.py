...
def run_single_turn(stimulus: dict, model_config: dict, settings: dict) -> dict:
    """Run all turns of a stimulus independently (single-turn mode) against one model."""
    system_prompt = stimulus.get(
    ...
    return {
        ...
        "theme": stimulus.get("theme"),
        "variant": stimulus.get("variant"),
        "goemotions_categories": stimulus.get("goemotions_categories"),
...
def _run_one(
    ...
    """Run one (stimulus, model, run) combination. Returns a result dict."""
    ...
    try:
        transcript = run_single_turn(stimulus, model_config, settings)
        ...
        for future in as_completed(futures):
            ...
            with counter_lock:
                if status == "skipped":
                    ...
                    total_input_tokens += result["transcript"]["total_usage"]["input_tokens"]
                    total_output_tokens += result["transcript"]["total_usage"]["output_tokens"]
                else:
                    total_errors += 1
                _completed = total_completed
